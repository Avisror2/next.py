...
# Draws lines between lists of dots with a specified color using the turtle package
def draw_lines(points, color):
    turtle.pencolor(color)
    # Starting position
    turtle.penup()
    turtle.goto(points[0], points[1])
    turtle.pendown()

    # Draw lines between points
    for i in range(2, len(points), 2):
        turtle.goto(points[i], points[i + 1])
...
def main():
    ...
    draw_lines(first, 'red')  # Passing the 'first' list and the color 'red'

    # Draw the second set of points
    draw_lines(second, 'blue')  # Passing the 'second' list and the color 'blue'
